...
static std::string symbol_name(int addr_space, int id) { return std::format("_symbol_addrspace{}_{}", addr_space, id); }
...
std::optional<std::string> HostEmitter::isa_targetspecific_intrinsic(ll::BB& bb, const Def* def) {
    ...
    if (auto default_stream = Axm::isa<gpu::default_stream>(def)) {
        ...
        auto [_, ins] = symbol_names_.emplace(std::format("@{}.name", sym_name));
        if (ins)
...
std::optional<std::string> DeviceEmitter::isa_targetspecific_intrinsic(ll::BB& bb, const Def* def) {
    ...
    if (auto mslot = Axm::isa<mem::mslot>(def)) {
        ...
        f += ll::internals::math_suffix(math_exp->type());
...
static std::optional<std::string> get_compute_capability() {
    ...
    return std::format("{}{}", out.substr(0, dot_pos), out.substr(dot_pos + 1));
}
...
void emit_host_with_embedded_device(World& world, std::ostream& ostream) {
    ...
    if (libdevice_stage) {
        ...
        // because any call operations (libdevice uses @__nvvm_reflect a lot for portability) are very costly on GPUs
        {
            ...
            auto cmd
                = std::format("{} {} {} -o {}", llvm_link, dev_ll_name, libdevice_path, libdevice_bc_name_unoptimized);
        ...
        }
        {
            ...
            auto cmd    = std::format("{} -passes=\"{}\" {} -o {}", opt, passes, libdevice_bc_name_unoptimized,
                                      libdevice_bc_name);
            auto rc     = sys::system(cmd);
            if (rc != 0) {
                println(std::cout, "Command existed with error code {}", rc);
    ...
    auto llc_input = libdevice_stage ? libdevice_bc_name : dev_ll_name;
    {
        ...
        auto cmd = std::format("{} -march=nvptx64 -mcpu=sm_{} {} -o {}", llc, comp_cap, llc_input, dev_ptx_name);
        auto rc  = sys::system(cmd);
        if (rc != 0) {
    ...
    }
    {
        ...
        auto cmd = std::format("{} -arch=sm_{} {} -o {}", ptxas, comp_cap, dev_ptx_name, dev_cubin_name);
        auto rc  = sys::system(cmd);
        if (rc != 0) {
            println(std::cout, "Command exited with error code {}", rc);
            return;
    ...
    }
    {
        ...
        auto cmd = std::format("{} --create={} -64 --image3=kind=elf,sm={},file={}", fatbinary, dev_fatbin_name,
                               comp_cap, dev_cubin_name);
        auto rc  = sys::system(cmd);
        if (rc != 0) {
            println(std::cout, "Command exited with error code {}", rc);
            return;
