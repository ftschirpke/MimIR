...
static bool is_gpu_type(const Def* type) {
    if (auto mem = Axm::isa<mem::M>(type)) {
        auto [_, addr_space] = mem->arg(0)->projs<2>();
        auto address_space   = Lit::as<plug::mem::AddrSpace>(addr_space);
        switch (address_space) {
            case mem::AddrSpace::Generic: return false;
            case mem::AddrSpace::Global: return true;
...
std::optional<std::string> DeviceEmitter::isa_targetspecific_intrinsic(BB& bb, const Def* def) { return std::nullopt; }
